  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        value: z.string(),
        parentId: z.string().uuid().nullish(), // optional
      })
    )
  ...
  getMany: baseProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [user] = await db
        .select()
        .from(users)
        .where(inArray(users.clerkId, clerkUserId ? [clerkUserId] : [])); // check if have userid
          ...
          .where(
            and(
              ...
              parentId
                ...
                : isNull(comments.parentId), // only comments without reply
              cursor
                ? or(
